/*
** $Id: listview.c 11323 2009-02-13 08:57:13Z tangjianbin $
**
...
static const BITMAP *bmp_fold, *bmp_unfold;
...
static void lvDrawHeaderCustom (HWND hWnd, HDC hdc)
{
    ...
    list_for_each (me, &plvdata->hdrqueue) {
        p1 = list_entry (me, LSTHDR, list);
        rcItem.left = p1->x - plvscr->nContX-1;
...
static void lvDrawHeader (HWND hWnd, HDC hdc)
{
    ...
    list_for_each (me, &plvdata->hdrqueue) {
...
#else
#if 0
        Draw3DControlFrameEx (hdc, hWnd, p1->x - plvscr->nContX + 1,
                            LV_HDR_TOP,
                            p1->x - plvscr->nContX + p1->width - 1,
        ...
        Draw3DControlFrameEx (hdc, hWnd, p1->x - plvscr->nContX + 1,
                            ...
                            p1->x - plvscr->nContX + p1->width - 1,
                            LV_HDR_TOP + LV_HDR_HEIGHT, 
                            flags | DF_3DBOX_FILL, GetWindowElementColorEx(hWnd, BKC_CONTROL_DEF));
#endif
        item_w += p1->width;
    ...
    DrawFlatControlFrameEx (hdc, hWnd, item_w - plvscr->nContX + 1,
            ...
            flags | DF_3DBOX_FILL, GetWindowBkColor (hWnd));
#else
    Draw3DControlFrameEx (hdc, hWnd, item_w - plvscr->nContX + 1,
            LV_HDR_TOP,
            rcClient.right+2,
            LV_HDR_TOP + LV_HDR_HEIGHT, 
            flags | DF_3DBOX_FILL, GetWindowBkColor (hWnd));
    ...
    case MSG_LBUTTONDOWN:
    {
        ...
        if (nCols >= 0) {  // clicks on the header
            ...
            if (plvdata->nHeadHeight > 0 && nCols >= 0) {
                ...
                if (pi->child && pt.x > indent && pt.x < indent + bmp_fold->bmWidth + 4) {
                    BOOL bfold;
                    bfold = (ISFOLD(pi)) ? FALSE : TRUE;
                    lvFoldItem (hWnd, plvdata, pi, bfold);
                    NotifyParentEx (hWnd, GetDlgCtrlID(hWnd), 
...
BOOL RegisterListViewControl (void)
{
    ...
    WndClass.iBkColor       = GetWindowElementColor (WEC_3DBOX_LIGHT);
