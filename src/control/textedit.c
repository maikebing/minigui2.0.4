/*
** $Id: textedit.c 11116 2008-11-03 00:53:37Z tangjianbin $
**
...
static int textdoc_get_paragraph_length (TextDoc *txtdoc, int index)
{
    list_t *me;
    TextNode *node;
    int i = 0;
    ...
    for (i = 0; i < line_nr; i++) {
        ...
        if (pos_start < fl.nr_chars && pos_end > 0) { /* second: selected section */
            ...
            if (!ptedata->drawSelected) {
                ...
                outw += TabbedTextOutLen (hdc, startx + outw, starty, content, outchars);
                if (!(ptedata->ex_flags & TEST_EX_SETFOCUS) && !(ptedata->ex_flags & TEST_EX_KILLFOCUS)) {
...
static void teSetCaretPos (HWND hWnd, PTEDATA ptedata)
{
    ...
    if (BE_WRAP(hWnd)) {
        ...
        */
        ...
        scrolled_make_pos_visible (hWnd, ptescr, -1, endh-1 > 0 ? endh-1 : 0);
#ifdef _SELECT_SUPPORT
        if (txtdoc->selection.curNode)
...
#endif
        {
            ...
            if ( ptedata->caretShape == ED_CARETSHAPE_BLOCK && 
                ...
                scrolled_make_pos_visible (hWnd, ptescr, -1, endh-1 + ptedata->nLineHeight);
                mySetCaretPos (hWnd, indent, h + ptedata->nLineAboveH + ptedata->nLineHeight);
            }
            else {
        ...
        GetTabbedTextExtent(hdc, (const char*)(node->content.string), mark->pos_lnOff, &txtsize);
        ptedata->des_caret_x = txtsize.cx + indent;
        endh = (h < ptescr->nContY) ? h : h + ptedata->nLineHeight;
        scrolled_make_pos_visible (hWnd, ptescr, 
                        txtsize.cx + indent, endh-1 > 0 ? endh-1 : 0);
#ifdef _SELECT_SUPPORT
        if (txtdoc->selection.curNode)
...
static int set_line_width (HWND hWnd, PTEDATA ptedata, TextNode *node, int indent)
{
    ...
    hdc = GetClientDC (hWnd);
    SelectFont (hdc, GetWindowFont(hWnd));
    GetTabbedTextExtent(hdc, (const char*)(node->content.string), node->content.txtlen, &txtsize);
    ReleaseDC (hdc);
...
#if defined(_CLIPBOARD_SUPPORT) && defined(_SELECT_SUPPORT)
...
static void teSetCaretPosOnChar (HWND hWnd, PTEDATA ptedata, int old_caret_pos)
{
    ...
    if (BE_WRAP(hWnd)) {
        ...
        GetTabbedTextExtent(hdc, (const char*)(node->content.string), old_caret_pos, &txtsize);
                 ...
                 int line, int char_pos, int *item_y, int *newpos)
{
    ...
    nr_chars = GetTextMCharInfo (GetWindowFont (hWnd),
                    (const char*)(markNode->content.string), markNode->content.txtlen, pos_chars);
...
static int TextEditCtrlProc (HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    case MSG_ERASEBKGND:
        return EditOnEraseBackground (hWnd, (HDC)wParam, (const RECT*)lParam);
...
*/
    ...
    case EM_GETPARAGRAPHLENGTH:
        return textdoc_get_paragraph_length (&ptedata->txtdoc, wParam);
