/*
** $Id: charset.c 11494 2009-04-09 08:48:03Z weiym $
** 
...
{
    if (mchar[1] > 0x7F)
        return (UChar32)gbkunicode_map[(mchar[0] - 0x81)*190 + mchar[1] - 0x41];
    else
        return (UChar32)gbkunicode_map[(mchar[0] - 0x81)*190 + mchar[1] - 0x40];
}
...
static UChar32 gb18030_0_conv_to_uc32 (const unsigned char* mchar)
{
    ...
    if (mchar [0] >= 0x90 && mchar [1] < 0x40) {
        return  ((mchar [0] - 144) * 12600) + 
                ((mchar [1] - 48) * 1260) +
                ((mchar [2] - 129) * 10) + 
                (mchar [3] - 48) + 65536;
    }

    return (UChar32)gb18030_0_unicode_map [gb18030_0_char_offset (mchar)];
}
...
#ifdef _UNICODE_SUPPORT
static UChar32 jisx0208_1_conv_to_uc32 (const unsigned char* mchar)
{
    unsigned short ucs_code;
