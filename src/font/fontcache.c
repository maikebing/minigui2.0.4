/*
** $Id: fontcache.c 7330 2007-08-16 03:17:58Z xgwang $
** 
...
struct _CacheSystem globalCache;
...
{
    new->nextCache = &(globalCache.queueDummyHead);
    new->prevCache = globalCache.queueDummyHead.prevCache;
    (globalCache.queueDummyHead.prevCache)->nextCache = new;
    globalCache.queueDummyHead.prevCache = new;
}
...
GetCache(int nblock, int ndir)
{
    ...
    if (globalCache.nCache == globalCache.maxCache) {
    ...
    temp->cache.memPool = malloc(globalCache.cacheSize);
    ...
    int nblk, int blksize, int ndir, MakeHashKeyFunc makeHashKey)
{
    ...
    if ((nblk * blksize) > globalCache.cacheSize) {
        return (HCACHE)(0);
    }
...
__mg_ttc_sys_init(int maxCache, int cacheSize)
{
    ...
    globalCache.queueDummyHead.prevCache = &globalCache.queueDummyHead;
    globalCache.queueDummyHead.nextCache = &globalCache.queueDummyHead;
    globalCache.maxCache = maxCache;
    globalCache.nCache = 0;
    globalCache.cacheSize = cacheSize;
    return 0;
}
...
__mg_ttc_sys_deinit(void)
{
    ...
    /* In this case, it means more than ONE cache in 
       ...
       that's all, may be~~ :(  */
    q = globalCache.queueDummyHead.nextCache;
    
    while (q != &globalCache.queueDummyHead) {
        pfree = q;
        q = q->nextCache;
...
__mg_ttc_is_exist(char *family, char *charset, DWORD style, int size)
{
    ...
    p = globalCache.queueDummyHead.nextCache;
    while (p != &globalCache.queueDummyHead) {
        if (strncmp(p->cache.family, family, LEN_FONT_NAME) == 0) {
            if (strncmp(p->cache.charset, charset, LEN_FONT_NAME) == 0) {
                if (p->cache.fontsize == size) {
                    int s = style & 0x0000000F;
                    if ((s == FS_WEIGHT_BOOK || s == FS_WEIGHT_DEMIBOLD) && 
...
void __mg_ttc_dump_all(void)
{
    CacheQueueNode* pc;
#if 0
    DP(("globalCache.maxCache = %d\n", globalCache.maxCache));
    DP(("globalCache.cacheSize = %d\n", globalCache.cacheSize));
    DP(("globalCache.nCache = %d\n", globalCache.nCache));
    
    DP(("globalCache.queueDummyHead.prevCache = %p\n", 
           globalCache.queueDummyHead.prevCache));
    DP(("globalCache.queueDummyHead.nextCache = %p\n",
           globalCache.queueDummyHead.nextCache));
#endif 
    pc = globalCache.queueDummyHead.nextCache;
    for ( ; pc != &globalCache.queueDummyHead; pc = pc->nextCache) {
