/*
** $Id: hi3560_fbvideo.c 7268 2007-07-03 08:08:43Z xwyan $
**
...
#ifndef _LITE_VERSION
struct GAL_VideoDevice *g_pVideo = NULL;
#endif
...
static int HI3560_AntiFlicker(_THIS, GAL_Surface *src, 
	                                GAL_Rect *srcrect, GAL_Surface *dst, 
	                                GAL_Rect *dstrect, Uint32 *puiOffsetX, 
	                                Uint32 *puiOffsetY);

static int HI3560_SetOffset(int fd, 
                                  Uint32 uiOffsetX, 
                                  Uint32 uiOffsetY);
...
static int ShowLogo (_THIS, Uint32* dst)
{
    ...
    param.out_stride = (U16)HI3560_ALIGN_4(__gal_screen->pitch);
    ...
    param.dst_stride = HI3560_ALIGN_4(param.out_stride);

    pvirtual_addr = (Uint8*)logosurface.pixels;

    HI3560_CHECK_VIRTUAL_ADDR(pvirtual_addr);    
    param.src_pixels = HI3560_GET_PHY(pvirtual_addr);
    param.src_stride = HI3560_ALIGN_4(logosurface.pitch);
    printMBParam(&param);
    ...
    #if 0
    {
      ...
      printf ("pitch = %d\n", HI3560_ALIGN_4(param.out_stride));
     
      for (i = 0; i < OutRect.h; i++)
      {
          memcpy(pdst, psrc, HI3560_ALIGN_4(param.out_stride));          
          psrc += HI3560_ALIGN_4(logosurface.pitch);
          pdst += HI3560_ALIGN_4(param.out_stride);
...
static int HI3560_HWAccelBlit(GAL_Surface *src, GAL_Rect *srcrect,
                       GAL_Surface *dst, GAL_Rect *dstrect)
{
...
#endif
    {
       //#ifdef ___DELETE___ANTIFLICKER__
...
#endif
	       {
	           HI3560_SetOffset(console_fd, uiOffsetX, uiOffsetY); 
	           return 0;
	       }
	   }
       //#endif
    ...
    if (src->flags & GAL_SRCCOLORKEY)
    {
        ...
        HI3560_GetRGB(src->format->colorkey.low, src->format, &r, &g, &b);
        param.ColorSpaceMin = MAKERGB(r, g, b);
        HI3560_GetRGB(src->format->colorkey.high, src->format, &r, &g, &b);
    ...
    HI3560_CHECK_VIRTUAL_ADDR(pvirtual_addr);
    param.src_pixels = HI3560_GET_PHY(pvirtual_addr);
    param.src_stride = HI3560_ALIGN_4(src->pitch);
    printMBParam(&param);
...
static int HI3560_GetAllAFVMem(_THIS, 
                                  Uint32 iPitch, 
                                  Uint32 iHeight,
...
/*该变量用于统计gui刷新屏幕的次数，主要用于启动画面到桌面，中间没有黑屏出现
将在DEMO中mginit.c文件中使用*/
int g_offset_counter = 0;
static int HI3560_SetOffset(int fd, 
                                  Uint32 uiOffsetX, 
                                  Uint32 uiOffsetY)
{
    ...
    if (++g_offset_counter == 3)
    {
    	Uint32 alpha = 255;
    	HI3560_SetLayerAlpha(fd, &alpha);
    }
...
{ 
    if ((0 > iLayerIndex) || (1 < iLayerIndex)
    	...
    	|| (0 == g_stHidden_3560[iLayerIndex].uiMiddleBuf1))
    {
        return -1;
    }
    TDE_WaitForDone();
    g_stHidden_3560[iLayerIndex].uiTapLevel = (Uint32)iLevel;   
   
    if (0 == iLevel)
    {                
        HI3560_SetOffset(g_stHidden_3560[iLayerIndex].console_fd1,
                         0, 
                        (g_stHidden_3560[iLayerIndex].uiCurrentBuf
                        - g_stHidden_3560[iLayerIndex].uiMiddleBuf1)/
                        g_stHidden_3560[iLayerIndex].finfo.line_length);
...
#endif
		vinfo.red  = s_fbargb[HI_GC_PF_RGB1555-1].red;
		vinfo.blue = s_fbargb[HI_GC_PF_RGB1555-1].blue;
		vinfo.green = s_fbargb[HI_GC_PF_RGB1555-1].green;		
		vinfo.transp = s_fbargb[HI_GC_PF_RGB1555-1].transp;
...
printf ("{%s}[%s](%d)\n", __FILE__, __func__, __LINE__);
    ...
    if ( ioctl(console_fd, FBIOGET_FSCREENINFO, &saved_finfo) < 0) 
    {
     ...
     || defined(_TAP2_VERT_HORI)|| defined(_TAP3_VERT)) \
     && defined(_USE_2D_ACCEL) && defined(_USE_DBUF)
    if (-1 == (iAFBufLen = HI3560_GetAllAFVMem(this,
   ...
   icount = 0;
            ...
            while(icount < 1000)
            {
               ...
               if (k == 0)
               {
                ...
                k = 1;
                printf("res:0\n");
	        HI3560_SetOffset(console_fd, 0, 0);
               }
               else
               {
                ...
                k = 0;
                printf("res:%d\n", vinfo.yres);
	        HI3560_SetOffset(console_fd, 0, vinfo.yres);
	    ...
	    {
printf ("{%s}[%s](%d)\n", __FILE__, __func__, __LINE__);
	        HI3560_SetOffset(console_fd, 0, iAFBufLen/saved_finfo.line_length);
	    }
	}
#endif
...
void* HI3560_MallocFromVram(int size)
{
    ...
    REP_HWSURFACE reply = {0, 0, NULL};
    Uint8* pBlockHead;
    Uint32 pitch = g_pVideo->screen->pitch;
...
void HI3560_FreeToVram(void* p)
{
    ...
    REP_HWSURFACE reply = {0, 0};
    fprintf(stderr, "hi3560 free vram\n");
