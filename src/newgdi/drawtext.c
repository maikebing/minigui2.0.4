/*
** $Id: drawtext.c 11285 2009-01-20 06:23:09Z houhuihua $
** 
...
int gdi_strnwrite (PDC pdc, int x, int y, const char* text, int len)
{
    ...
    if (pdc->bkmode != BM_TRANSPARENT) {
        ...
        /* houhh 20090120, bugfix for glyph outside the back area. */
        if (pdc->pLogFont->style & FS_WEIGHT_BOOK_LIGHT) {
            gal_rc_bg.x--;    gal_rc_bg.y--;
            gal_rc_bg.w += 2; gal_rc_bg.h += 2;
        }
