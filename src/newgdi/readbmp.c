/*
** $Id: readbmp.c 10848 2008-08-27 02:43:06Z weiym $
**
...
BOOL GUIAPI InitBitmapPixelFormat (HDC hdc, PBITMAP bmp)
{
    ...
    if (bmp->bmType & BMP_TYPE_ALPHA) {
        ...
        bmp->bmAlphaPixelFormat = GAL_AllocFormat (bmp->bmBytesPerPixel << 3,
                ...
                const MYBITMAP* my_bmp, RGB* pal, BOOL alloc_all)
{
    ...
    if (my_bmp->flags & MYBMP_ALPHA && my_bmp->depth == 32) {
        ...
        else switch (bmp->bmBytesPerPixel) {
            case 2:
                Rmask = 0x0000F000;
                Gmask = 0x00000F00;
                Bmask = 0x000000F0;
        ...
        if (bmp->bmBitsPerPixel > 8) {
            ...
            bmp->bmAlphaPixelFormat 
                    = GAL_AllocFormat (bmp->bmBytesPerPixel << 3, 
                                        Rmask, Gmask, Bmask, Amask);
            if (!bmp->bmAlphaPixelFormat)
            {
                ret = ERR_BMP_MEM;
...
#else
                    dp2 [x*3] = dp1 [(sx >> 16) * 3];
                    dp2 [x*3 + 1] = dp1 [(sx >> 16) * 3 + 1];
                    dp2 [x*3 + 2] = dp1 [(sx >> 16) * 3 + 2];
#endif
                    sx += xfactor;
                    x++;
                }
