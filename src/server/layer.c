/*
** $Id: layer.c 7360 2007-08-16 05:09:24Z xgwang $
** 
                ...
                int nr_topmosts, int nr_normals)
{
    ...
    layer->zorder_shmid = shmget (shm_key, 
...
BOOL GUIAPI ServerDeleteLayer (MG_Layer* layer)
{
    ...
    if (layer == NULL || strcmp (layer->name, NAME_DEF_LAYER) == 0) {
        printf ("The MiniGUI Server: Can not delete the default layer.\n");
        return FALSE;
    }

    if (zi->nr_topmosts + zi->nr_normals + zi->nr_popupmenus != 0) {
        printf ("The MiniGUI Server: Some objects are still in this layer.\n");
        return FALSE;
    }
